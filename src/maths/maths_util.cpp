...
vector<RCPoint> fft( vector<RCPoint> x )
{
    ...
    ret_val_complex.resize( N, RCPoint( 0, 0 ) );
...
{
    int N = domain.size();
    if ( domain.size() != X.size() )
        throw "Domain and function must be the same size!\n";
    int M = 1;
    while ( M < N ) M <<= 1; // find the smallest power of two greater than N
    M >>= 1;
...
vector<RCPoint> FourierSpaceDerivative( vector<RCPoint> X )
{
    ...
    for ( int n = 0; n < N; ++n )
        X_prime[ n ] = RCPoint( X[ n ].x, X[ n ].y * complex( 0.f, 2.f * glm::pi<float>() * n / N ) );
    return X_prime;
}