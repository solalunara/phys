    ...
    dir( CartnBasisVector ), Length( Length ), Step( Step )
{
    ...
    {
        case i_hat:
        AxisLabel = new GameText( "x", Origin.x + Length / 2.0f, Origin.y - .7f, Origin.z, 0.01, vec3( .8f, .8f, .8f ), container );
        break;

        case j_hat:
        AxisLabel = new GameText( "y", Origin.x - .7f, Origin.y + Length / 2.0f, Origin.z, 0.01, vec3( .8f, .8f, .8f ), container );
        break;

        case k_hat:
        AxisLabel = new GameText( "z", Origin.x, Origin.y - .7f, Origin.z + Length / 2.0f, 0.01, vec3( .8f, .8f, .8f ), container );
...
#define NUMLABEL_OFFSET .3f
    for ( int i = 0; i < BoxNum; ++i )
    {
        switch ( CartnBasisVector )
        {
            ...
            ValueLabels.push_back( new GameText( std::to_string( i * Step - Length ).c_str(), Origin.x + i * Step - Length, Origin.y - NUMLABEL_OFFSET, Origin.z, 0.01, vec3( 1, 1, 1 ), container ) );
            break;
            ...
            ValueLabels.push_back( new GameText( std::to_string( i * Step - Length ).c_str(), Origin.x - NUMLABEL_OFFSET, Origin.y + i * Step - Length, Origin.z, 0.01, vec3( 1, 1, 1 ), container ) );
            break;
            ...
            ValueLabels.push_back( new GameText( std::to_string( i * Step - Length ).c_str(), Origin.x, Origin.y - NUMLABEL_OFFSET, Origin.z + i * Step - Length, 0.01, vec3( 1, 1, 1 ), container ) );
